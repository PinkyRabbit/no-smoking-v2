...
export const dialogsRu: MultilineContent = {
  ...
  [Content.ON_IDLE_EASY_LEVEL]: `
    📌 Вы сейчас используете ${B}Базовый уровень сложности${B}${N2} 
    На этом уровне ${B}штрафные баллы не учитываются${B}, чтобы вам было проще разобраться в том, как работает бот.${N2}
    Если чувствуете, что уже освоились, попробуйте включить их, нажав /level
  `,
  [Content.DIFFICULTY]: `
    📈 ${B}Выберите уровень сложности${B}${N2}
    В приложении доступно три уровня сложности:${N2}
    - ${B}Базовый${B} (учебный): +0.5 секунды в день${NL}
    - ${B}Обычный${B}: +1 секунда в день${NL}
    - ${B}Продвинутый${B}: +2 секунды в день${N2}
    ${B}Базовый${B} подходит для начинающих, чтобы привыкнуть соблюдать интервалы, 
  ...
  `,
  [Content.DIFFICULTY_EASY]: "Базовый",
  ...
  [Content.DIFFICULTY_AUTO]: `
    ...
    Вам установен БАЗОВЫЙ уровень сложности${ND}
    ${B}Почему важно начать с "Базового" уровня?${B}${N2}
    ...
    ${I}Базовый этап создан как раз для того чтобы решить эту проблему.${I}${ND}
    ...
    ⬆️ ⬆️ ⬆️ ⬆️ ⬆️${NL}
    Настройка вашего аккаунта будет автоматически завершена через 2 минуты. 
  ...
  [Content.PENALTY_3]: `
    ...
    ${B}Вы должны ограничить себя сами:${B}${N2}
    - Не курите раньше времени${NL}
    - Не курите по два раза никогда${NL}
    - Старайтесь отказаться от привычек, таких как ${I}перекур утром, перед сном, 
    за кофе, в паузах между работой или когда ждёте трансопрт, и так далее${I} 💪${NL}
    ...
    ${B}Помните, что привычка - это не зависимость.${NL} Вы сильнее этого!${B}
  `,
